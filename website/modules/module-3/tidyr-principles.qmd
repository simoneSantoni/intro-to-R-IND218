...
### `separate_wider_delim()`: Split One Column into Multiple

Use `separate_wider_delim()` when one column contains several variables separated
by a known delimiter. Supply the source column, the new column names, and the
delimiter. The function is the modern replacement for `separate()` and reports
more informative errors when a value has too many or too few pieces.
...
separated_data <- combined_data %>%
  separate_wider_delim(employee, delim = "_", names = c("first", "last")) %>%
  separate_wider_delim(
    dept_role,
    delim = "_",
    names = c("department", "role")
  ) %>%
  separate_wider_delim(date_time, delim = "_", names = c("date", "time"))
...
survey_clean <- survey_raw %>%
  ...
  separate_wider_delim(
    demographics,
    delim = ",",
    names = c("age_group", "gender", "location")
  ) %>%
...
- **`separate_wider_delim()`/`unite()`**: Split and combine columns
- **`complete()`**: Fill in missing combinations
