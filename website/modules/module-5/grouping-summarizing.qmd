...
Current dplyr also supports per-operation grouping. It is useful when only one verb needs groups because the output does not retain grouped metadata:
...
Read this from the inside out: `n()` counts rows, `n_distinct()` counts customers, and `sum()` aggregates revenue. Because one customer can place several orders, the first two indicators have different denominators. Use `group_by()` when several following verbs genuinely need the same grouping; use `.by` for a local summary.

::: {.callout-tip title="Attach uncertainty and context"}
A KPI without its denominator is easy to misread. Report counts beside averages, inspect small groups, and compare distributions rather than relying only on means. An impressive average based on two orders should not drive the same decision as one based on two thousand.
:::
